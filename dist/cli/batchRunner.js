...
function simulateOneSeed(config, mode, rounds, seed) {
...
function main() {
    ...
    for (let i = 0; i < seedsCount; i++) {
        ...
        all.push(r);
        console.log(`[${i + 1}/${seedsCount}] seed=${seed} | RTP(base)=${pct(r.rtpPerBase, 3)}% | RTP(cost)=${pct(r.rtpPerCost, 3)}% | RPS=${r.rps.toFixed(0)}`);
    }
...
Cap Hits (sum over all seeds):
  Capped Rounds (>= ${maxWinXRule}): ${all.reduce((a, r) => a + r.cappedRounds, 0)}
...
`.trim();
    ...
    fs_1.default.writeFileSync(csvPath, toCsv(all), "utf8");
    fs_1.default.writeFileSync(txtPath, report + "\n", "utf8");
