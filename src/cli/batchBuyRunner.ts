...
type SeedRow = {
  ...
  avgWinX: number;
  rtpPerCostPct: number;

  hitRatePct: number;
  breakEvenRatePct: number;
  profit2xRatePct: number;

  maxWinX: number;
  cappedCount: number;
  cappedRatePct: number;

  // optional if you track these in playBonusBuy later
  // avgBonusSpins?: number;
  // avgRetriggers?: number;
...
const RUNS_PER_SEED = Number(process.argv[4] ?? 200_000);
const SEEDS = Number(process.argv[5] ?? 20);
const SEED0 = Number(process.argv[6] ?? 1337);

const maxWinXRule = config.rules?.maxWinX ?? 10_000;
...
for (let sidx = 0; sidx < SEEDS; sidx++) {
  ...
  for (let i = 0; i < RUNS_PER_SEED; i++) {
    const r = playBonusBuy(config, rng, buyType);
  ...
  console.log(
    `[${(sidx + 1)
      ...
      .padStart(
        2,
        "0"
      )}/${SEEDS}] seed=${seed} | RTP(cost)=${rtpPerCostPct.toFixed(
      3
    )}% | BE=${breakEvenRatePct.toFixed(2)}% | 2x+=${profit2xRatePct.toFixed(
      2
    )}% | cap=${capped} | RPS=${Math.round(rps)}`
  );
}
...
fs.writeFileSync(outJson, JSON.stringify({ rows, summary }, null, 2), "utf8");
