...
const mode = (process.argv[3] ?? "base") as "base" | "extraBet";
...
for (let i = 1; i <= ROUNDS; i++) {
  ...
  if (rr.triggered === "bonus3") {
    ...
    if (rr.bonusWinX >= maxWinXRule) cappedBonus3++; // falls du Bonus3 so hart cappend machst
  ...
  if (i % PROGRESS_EVERY === 0) {
    ...
    console.log(
      `[${i.toLocaleString()} / ${ROUNDS.toLocaleString()}] ` +
...
Cap Hits:
  Capped Rounds (>= ${maxWinXRule}): ${cappedRounds} (${(
  (cappedRounds / ROUNDS) *
  100
).toFixed(4)} %)
...
console.log(report);
fs.writeFileSync(
  `stats_round_${config.id}_${mode}_${ROUNDS}_${SEED}.txt`,
  report
);
